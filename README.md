...
For each file
      ...
      +----> Read blob content
      |
      +----> HTTP POST to downstream purchase-order service
      |
      +----> Write HTTP response to /omspo
      |
...
HTTP POST
   |
   v
Create /omspo/oms_<timestamp>.json
   |
   v
Delete original source file
```

The action dependencies in the workflow enforce this sequence.
